jQuery(function(){
  if(jQuery("[zoom]").length > 0){
  	...
  	var setZoom = function () {
  		...
  		jQuery("<img/>")
  			...
  			.load(function () {
  				...
  				jQuery('.zoom-mouse-catcher-' + id).remove();
  				jQuery('.zoom-len-' + id).remove();
  				...
  				mouseCatcher.mousemove(function (e) {
  					...
  					actualLeft = jQuery(element).position().left;
  					actualTop = jQuery(element).position().top;
  	...
  	var setMouseCatcher = function () {
  		var mouseCatcher = jQuery('.zoom-mouse-catcher-' + id);
  	...
  	var watchResize = jQuery(window).resize(function ($event) {
  		imgWidth = cardImg.width();
  		imgHeight = cardImg.height();
  		actualLeft = parseInt(jQuery(element).position().left);
  		actualTop = parseInt(jQuery(element).position().top);
  		setMouseCatcher();
  	});
  	...
  	observer.observe(jQuery(element).get(0), config);
